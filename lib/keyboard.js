...
Keyboard.prototype._configure = function (camera) {
  ...
  this.keyMap[32] = Keyboard.Actions.UP; //space
  this.keyMap[17] = Keyboard.Actions.DOWN; //ctrl
  ...
  for (var k in this.keyPressed) {
    if (this.keyPressed[k]) {
      switch (this.keyMap[k]) {
        ...
        case Keyboard.Actions.UP: ymove -= 1; break;
        case Keyboard.Actions.DOWN: ymove += 1; break;
      }
    }
  }
  logger.log("xmove : " + xmove + ", zmove : " + zmove + ", ymove : " + ymove);
  this.camera.move(zmove, xmove, ymove, frameTime);
};
